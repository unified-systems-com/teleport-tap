...
- **`/teleport`** — a deployment graph (layout module `static/teleport/js/projections/teleport-deployment.js`) over seven board sections: posture, roles, identity, resources, requests, machines, trust. `?cluster=<cluster name>` picks the cluster by its name (for example `/teleport?cluster=teleport.staging`); with no `?cluster` every cluster is in scope, and one cluster on the grid is chosen automatically.
