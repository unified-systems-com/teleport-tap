...
`grift/page.grift.json` (bundle `page`, batch "teleport page v0.3.0"): one page, full-bleed, one column —
...
**Scene searches take the name.** Every search declares one input, `cluster` (`["string", "null"]`, default `null`), and filters each cluster it reaches with `($cluster IS NULL OR c.data.name = $cluster)`, so `?cluster` absent returns every cluster (the one cluster on a single-cluster grid, the picker on a multi-cluster one). Before tap v0.2.2 Gryphon had no parameter-absent predicate (tap#360) and the searches defaulted to the cluster type's slug as an every-cluster sentinel; that workaround is retired. This is the Okta page's pattern (okta-tap `specs/spec-okta-v0.md` § Page: Org).

- **Both ends in the cluster.** A search that joins two Teleport records closes the loop on one cluster variable, `(c)<-[:BELONGS_TO_CLUSTER]-(a)-[:EDGE]->(b)-[:BELONGS_TO_CLUSTER]->(c)` (`CALLS_AUTH_API`, `DIALS_REVERSE_TUNNEL`, `SERVES_RESOURCE`). A repeated variable binds one node (tap#780, fixed in tap v0.2.2), so a cross-cluster edge never pulls a foreign proxy, auth server or resource onto this cluster's picture, including when `?cluster` is absent and every cluster is shown. A search whose far end is another platform's record (`RUNS_ON_COMPUTE`, `STORES_CLUSTER_STATE`, `WRITES_AUDIT_EVENTS`, `UPLOADS_SESSION_RECORDINGS`, `SIGNS_WITH_KEY`, `FRONTS_TARGET`) scopes its Teleport end through membership; the open end carries no cluster. `TRUSTS_ROOT_CLUSTER` joins two clusters, so it keeps an edge with the chosen cluster at *either* end and draws the peer.
...
| req-teleport-page-6 | Scoped By Cluster Name | Implemented | Every scene search takes `cluster` (nullable, default `null`, tested by `$cluster IS NULL`); with two clusters and an edge of every drawn type crossing between them, each search returns only the named cluster's records, in both directions; `?cluster=a` does not match a cluster named `aba`; absent, every cluster is returned and an edge search still keeps both ends in one cluster. | `tests/test_teleport_page.py` |
