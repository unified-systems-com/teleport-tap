# GRANTS_RESOURCE_ACCESS

## Blurb

A role's allow rules reach this protected resource: its `*_labels` selectors match the resource's labels and no deny rule of the same role excludes it. Derived, not declared by Teleport; the property says how, so a view can tell label-matched from wildcard access.
...
- **Source:** `teleport__teleport_role`
- **Target:** `teleport__teleport_ssh_node`, `teleport__teleport_kube_cluster`, `teleport__teleport_database`, `teleport__teleport_app`, `teleport__teleport_windows_desktop`
- **Dimensions:** `teleport.plane: policy`

Properties:

- `principals` — What the grant lets its holder be on the resource: SSH logins, Kubernetes groups, database users, Windows logins. Empty when the role grants the resource but names no principal.
- `match` — `labels` when a specific selector matched; `wildcard` when the selector was `'*': '*'`.
