...
A proxy server calls an auth server's gRPC API for every authentication and authorization decision (through the auth load balancer in HA). Declared per pair so a design can show that each proxy reaches the auth pool.
...
- **Source:** `teleport__teleport_proxy_server`
- **Target:** `teleport__teleport_auth_server`
