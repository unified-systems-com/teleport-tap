# PERMITS_ROLE_REQUEST

## Blurb

A role lets its holders request another role (`allow.request.roles`) — the just-in-time path to elevated access.
...
- `approvals_required` — Approvals the request needs (the role's request thresholds); absent when not observed.
