...
- `join_method` — How a joiner proves itself: `token` (shared secret), `ec2`, `iam`, `github`, `gitlab`, `kubernetes`, `azure`, `gcp`, `tpm`, `circleci`, `spacelift`, `terraform_cloud`, `bitbucket`, `oracle`, `azure_devops`, `bound_keypair` …. A string because Teleport adds methods most releases, but lower-case `snake_case` only, as Teleport spells them, so a mis-cased `Token` cannot slip past the secret-digest rule. Required: a design decides it.
