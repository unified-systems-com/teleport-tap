...
An agent dials a proxy and holds a reverse tunnel open, so the proxy can reach the agent's resources without any inbound port on the agent's network.
...
- **Target:** `teleport__teleport_proxy_server`
