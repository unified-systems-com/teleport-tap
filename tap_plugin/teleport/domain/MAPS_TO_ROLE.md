...
An SSO connector maps an IdP attribute (SAML attribute, OIDC claim, GitHub org/team) value to a Teleport role at login.
...
- `attribute` — The IdP attribute, claim or `org/team` the mapping reads.
- `value` — The value (or glob/regex) that must match.
